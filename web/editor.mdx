...
The Queries panel in the editor shows every database query discovered in your app's codebase, organized by resource. For each query you can see:

- The invocation key used to call it.
- The file location where it appears.
- The full query string, which you can edit directly in the panel.

Queries are extracted automatically when commits are pushed to the main branch of your app's repository. Open the Queries panel from the editor toolbar to review or update query strings without hunting through source files.
