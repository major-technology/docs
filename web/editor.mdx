...
The editor includes a **Queries** panel that lists every database query discovered in your application's source code. Queries are organized by resource and include metadata such as the invocation key and the file location where the query originates. You can edit query strings directly from this panel.

Major populates the Queries panel automatically when commits are pushed to your main branch via GitHub webhook integration. No manual step is required after setup.
