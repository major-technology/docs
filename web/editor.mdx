...
Sessions persist across reconnections. When you resume a session on the same app, your conversation history and workspace state are automatically restored, so you pick up right where you left off without re-cloning or reinstalling dependencies.

To start the agent fresh on an unrelated task, say "clear context" in chat. The agent will reset its context window while keeping the code changes already in your repository intact.

## Debugging Logs

The AI assistant can query logs from both your live preview and your deployed application.

- **Preview logs** — logs from the local dev server running in your current session. Use these to debug build errors, hot-reload issues, or runtime problems during development.
- **Production logs** — logs collected from your deployed app's container stdout/stderr. Use these to investigate issues in production.

Ask the AI to investigate an error and it will search the appropriate log source automatically, correlate messages with your code, and suggest fixes.
