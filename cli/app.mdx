...
| Flag | Description |
| --- | --- |
| `--since` | Start of the time window (ISO date or relative, e.g. `2h`). |
| `--until` | End of the time window. |
| `--search` | Filter to lines matching this substring. |
| `--next-token` | Pagination token returned by a previous call. |
