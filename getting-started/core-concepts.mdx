...
Need a database for your app? Major can provision and host one for you — no external setup required. Ask the AI assistant to provision a managed connector and your app gets its own PostgreSQL instance, ready to use with the same generated clients as any other connector. Managed connectors are tied to your app's lifecycle and can be configured per-environment.
